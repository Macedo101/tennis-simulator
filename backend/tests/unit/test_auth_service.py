...
async def test_first_registered_user_becomes_admin(auth_service: AuthService) -> None:
...
async def test_second_registered_user_is_not_admin(auth_service: AuthService) -> None:
    await auth_service.register(
        email="primeiro@example.com", password="SenhaForte123!", full_name="Primeiro Utilizador"
    )

    second_user = await auth_service.register(
        email="segundo@example.com", password="SenhaForte123!", full_name="Segundo Utilizador"
    )
