...
async def get_simulation(
    ...
    if simulation.user_id != current_user.id and current_user.role != "admin":
...
async def simulation_progress_ws(
    ...
    não permite cabeçalhos customizados, pelo que o token vai na própria
    URL de ligação (`websocket_url` devolvido por `POST /simulations`
    já inclui isto, ver frontend). A mesma verificação de posse do `GET
    /simulations/{id}` aplica-se aqui (dono ou admin).
    """
    await websocket.accept()
    try:
        if token is None:
            await websocket.close(code=4401, reason="Token em falta.")
            return
