...
class AuthService:
    ...
    async def register(self, *, email: str, password: str, full_name: str) -> User:
        ...
        O primeiro utilizador alguma vez registado torna-se `admin`
        automaticamente — sem isto, não haveria forma de criar o
        primeiro administrador num deployment novo sem acesso direto à
        base de dados (não existe nenhum endpoint de "promover a
        admin", por design: um endpoint desses seria ele próprio um
        risco de segurança sem já existir um admin para o autorizar).
        """
        existing_users_count = await self._users.count()
